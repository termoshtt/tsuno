...
impl BasisRepresentation {
    ...
    pub(super) fn solve(&self, rhs: &Array1<f64>) -> Result<Array1<f64>, BasisError> {
        match self {
            BasisRepresentation::Factorized(lu) => lu.solve(rhs).map_err(BasisError::Lu),
    ...
    pub(super) fn solve_transposed(&self, rhs: &Array1<f64>) -> Result<Array1<f64>, BasisError> {
        match self {
            BasisRepresentation::Factorized(lu) => lu.solve_transposed(rhs).map_err(BasisError::Lu),
            BasisRepresentation::LessEqualSlack { base, basis_row } => {
                let base_dimension = base.dimension();
